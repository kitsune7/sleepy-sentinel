...
- [x] **Collapse W&B tracking to one run per training command.**
...
- [x] **Replace per-fold confusion matrix files with one combined artifact.**
...
- [x] **Introduce a run output contract.**
...
- [x] **Make logs high-level by default.**
...
- [x] **Separate artifact writing from experiment orchestration.**
...
- [x] **Separate tracking from training.**
...
- [x] **Create explicit run/config objects.**
...
- [x] **Decide whether the neural model earns default status.**
...
- [x] **Remove or finish the ordinal-model stub.**
...
- [x] **Give baseline/model runs a shared evaluation interface.**
...
- [x] **Keep diagnostics grouped, not fragmented.**
...
- [x] **Replace extraction `print` calls with level-based logging.**
...
- [x] **Fix extraction CLI validation flow.**
...
- [x] **Reduce broad `Any` usage where the data shape is stable.**
...
- [x] **Update tests so they protect the quieter contract.**
...
- [x] **Align docs with the simplified default command.**
