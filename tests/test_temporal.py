...
def test_pad_sequences_pads_and_records_lengths() -> None:
    ...
    padded, lengths = temporal.pad_sequences([short, long])
...
def test_gru_classifier_ignores_padding() -> None:
    ...
    torch.manual_seed(0)
    model = temporal.GruVideoClassifier(input_dim=4, hidden_dim=8, dropout=0.0)
    model.eval()

    sequence = np.random.default_rng(0).normal(size=(5, 4)).astype(np.float32)
    alone, alone_lengths = temporal.pad_sequences([sequence])
    with_pad, pad_lengths = temporal.pad_sequences([sequence, np.zeros((9, 4), dtype=np.float32)])
