...
def run_cross_validation(
    ...
    *,
    ...
    metric_summary_df = fold_metrics_df.groupby("fold")[["qwk", "rank_mae", "accuracy", "macro_f1"]].agg(
        ["mean", "std"]
    )

    confidence_tables = []
    error_tables = []
    for fold_idx, fold_predictions in video_predictions_df.groupby("fold"):
        confidence_table = metrics.confidence_summary(fold_predictions)
        confidence_table.insert(0, "fold", fold_idx)
        confidence_tables.append(confidence_table)

        error_table = metrics.error_slice_summary(fold_predictions, "label")
        error_table.insert(0, "fold", fold_idx)
